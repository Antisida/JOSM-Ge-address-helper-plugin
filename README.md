...
Функция не трогает уже существующие в редакторе данные, т.е удаляются только загруженные из NAPR в данном запросе дубли.
Поэтому проставление дублирующего адреса возможно, когда в слое существует здание с адресом и оно меньше по площади, чем здание, для которого
...
При обнаружении дубликата адреса создается точка с тегом "DOUBLE"="ADDRESS".
