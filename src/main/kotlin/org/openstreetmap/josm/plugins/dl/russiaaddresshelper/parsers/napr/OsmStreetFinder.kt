...
fun main() {
    val candidates = listOf(
        "улица Михаила Лермонтова", // Вариант А: 3 слова, полное имя
        "улица Лермонтова",         // Вариант Б: 2 слова
        "проспект Лермонтова"       // Вариант В: другое статусное слово
    )

    // Тест 1: Запрос "улица Лермонтов"
    // Подходят оба: "улица Михаила Лермонтова" и "улица Лермонтова".
    // Но "улица Лермонтова" ближе, так как совпадает количество слов (нет лишнего "Михаила").
    val query1 = "улица Лермонтов"
    println("Запрос: \"$query1\" -> Лучший: \"${findBestMatchingAddress(query1, candidates)}\"")
    // Ожидаем: улица Лермонтова

    // Тест 2: Запрос "улица М. Лермонтова"
    // К "улица Лермонтова" не подходит (лишнее слово "М.").
    // К "улица Михаила Лермонтова" подходит идеально через инициал.
    val query2 = "улица М. Лермонтова"
    println("Запрос: \"$query2\" -> Лучший: \"${findBestMatchingAddress(query2, candidates)}\"")
    // Ожидаем: улица Михаила Лермонтова
}

fun getOsmStreetName(dataSet: DataSet) : List<String> {
    return dataSet.allNonDeletedCompletePrimitives()
        .filter { p ->
            p.hasKey("highway")
                    && p.hasKey("name")
...
/**
 * Модель оценки совпадения. Реализует Comparable для удобной сортировки.
 */
data class MatchScore(
    val exactMatchesCount: Int,     // Чем больше точных совпадений, тем лучше
    val wordCountDiff: Int,         // Чем меньше разница в длине, тем лучше
    val totalLevenshtein: Int,      // Чем меньше опечаток, тем лучше
    val initialMatchesCount: Int    // Чем меньше инициалов (и больше полных слов), тем лучше
) : Comparable<MatchScore> {

    override fun compareTo(other: MatchScore): Int {
        // 1. Сравниваем точные совпадения (больше = лучше)
        val exactComp = this.exactMatchesCount.compareTo(other.exactMatchesCount)
        if (exactComp != 0) return exactComp

        // 2. Сравниваем разницу в количестве слов (меньше = лучше, поэтому inverted)
        val diffComp = other.wordCountDiff.compareTo(this.wordCountDiff)
        if (diffComp != 0) return diffComp

        // 3. Сравниваем расстояние Левенштейна (меньше = лучше, поэтому inverted)
        val levComp = other.totalLevenshtein.compareTo(this.totalLevenshtein)
        if (levComp != 0) return levComp

        // 4. Сравниваем использование инициалов вместо полных слов (меньше инициалов = лучше)
        return other.initialMatchesCount.compareTo(this.initialMatchesCount)
    }
}
