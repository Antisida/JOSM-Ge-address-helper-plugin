...
//import org.openstreetmap.josm.plugins.dl.geaddresshelper.api.N_ParsedAddresses
...
//            val nParsedAddresses : N_ParsedAddresses? = entry.value.addressInfo
//            val parsedAddressList = nParsedAddresses?.addresses ?: listOf()
//            for (parsedAddress in parsedAddressList) {
//                if (nParsedAddresses?.getPreferredAddress() == parsedAddress) {
//                    if (parsedAddress.flags.contains(ParsingFlags.STREET_NAME_FUZZY_MATCH) && !primitive.hasTag("addr:street")
//                        && !entry.value.isIgnored(EGRNTestCode.EGRN_STREET_FUZZY_MATCHING)) {
//                        val parsedStreetName = parsedAddress.parsedStreet.extractedType?.name + " " + parsedAddress.parsedStreet.extractedName
//                        val mostRelevantOsmName = parsedAddress.parsedStreet.mostRelevantOsmName
...
//                        affectedPrimitives = affectedPrimitives.plus(parsedAddress.parsedStreet.matchingPrimitives.toSet())
...
//            errorPrimitives.forEach{geaddresshelperPlugin.cache.markProcessed(it, EGRNTestCode.EGRN_STREET_FUZZY_MATCHING)}
...
//                val addressInfo = geaddresshelperPlugin.cache.get(it)?.addressInfo
...
//                val prefferedAddress = geaddresshelperPlugin.cache.get(it)!!.addressInfo?.getPreferredAddress()
