...
//
///**
// * ЕГРН граница места неполная Распознанный адрес сопоставлен с границей места, нарисованной в ОСМ мультиполигоном.
// * Но этот мультиполигон загружен не до конца и проверить адрес на вхождение в границу не получается.
// * Надо догрузить границу вручную.
// *
// * ЕГРН вне границ места Дом с распознанным адресом не лежит внутри полигона/мультиполигона границы.
// *
// * ЕГРН место слишком далеко Адрес дома сопоставлен с местом, у которого нет (мульти)полигональной границы,
// * а точка места находится слишком далеко (дальше заданного в настройках значения).
// *
// * ЕГРН улица слишком далеко Адрес дома сопоставлен с линией улицы,
// * которая находится слишком далеко (дальше заданного в настройках значения).
// * Это может быть одноименная улица в соседнем НП, или же линия улицы требует продолжения.
// */
...
//        val response = geaddresshelperPlugin.cache.get(p)
...
//                Logging.warn("EGRN PLUGIN: Parent too far validator got street name ${parsedStreet.mostRelevantOsmName}, but have no matched primitives! ")
//                return
//            }
...
//            ) {
//                val streetName = parsedStreet.mostRelevantOsmName
//                geaddresshelperPlugin.cache.markProcessed(p, EGRNTestCode.EGRN_STREET_FOUND_TOO_FAR)
...
//            val centroidNode = Node(GeometryHelper.getPrimitiveCentroid(p))
//            if (matchedNodes.isNotEmpty() && !geaddresshelperPlugin.cache.isIgnored(
//                    p,
...
//                    && !geaddresshelperPlugin.cache.isIgnored(p, EGRNTestCode.EGRN_ADDRESS_NOT_INSIDE_PLACE_POLY)
//                ) {
...
//                    if (relation.hasIncompleteMembers() && !geaddresshelperPlugin.cache.isIgnored(
//                            p,
...
//                            && !geaddresshelperPlugin.cache.isIgnored(
//                                p,
...
//        val parsedResponse = geaddresshelperPlugin.cache.get(primitive)
...
//                osmObjectName = parsedAddress.getPreferredAddress()!!.parsedStreet.mostRelevantOsmName
//                errorText =
...
//        }
//        if (answer == 1) {
//            geaddresshelperPlugin.cache.ignoreValidator(
